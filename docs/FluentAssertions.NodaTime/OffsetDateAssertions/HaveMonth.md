# OffsetDateAssertions.HaveMonth method

Asserts that the current OffsetDate has the specified month.

```csharp
public AndConstraint<OffsetDateAssertions> HaveMonth(int month, string because = "", 
    params object[] becauseArgs)
```

| parameter | description |
| --- | --- |
| month | The month that the current OffsetDate is expected to have. |
