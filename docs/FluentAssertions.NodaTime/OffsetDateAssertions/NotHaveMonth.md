# OffsetDateAssertions.NotHaveMonth method

Asserts that the current OffsetDate does not have the specified month.

```csharp
public AndConstraint<OffsetDateAssertions> NotHaveMonth(int month, string because = "", 
    params object[] becauseArgs)
```

| parameter | description |
| --- | --- |
| month | The month that the current OffsetDate is not expected to have. |
