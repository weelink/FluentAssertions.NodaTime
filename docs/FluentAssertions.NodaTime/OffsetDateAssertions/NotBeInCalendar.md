# OffsetDateAssertions.NotBeInCalendar method

Asserts that the current OffsetDate does not have the specified CalendarSystem.

```csharp
public AndConstraint<OffsetDateAssertions> NotBeInCalendar(CalendarSystem calendar, 
    string because = "", params object[] becauseArgs)
```

| parameter | description |
| --- | --- |
| calendar | The CalendarSystem that the current OffsetDate is not expected to have. |
