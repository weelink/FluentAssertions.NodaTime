# OffsetDateAssertions.NotHaveEra method

Asserts that the current OffsetDate does not have the specified era.

```csharp
public AndConstraint<OffsetDateAssertions> NotHaveEra(Era era, string because = "", 
    params object[] becauseArgs)
```

| parameter | description |
| --- | --- |
| era | The era that the current OffsetDate is not expected to have. |
