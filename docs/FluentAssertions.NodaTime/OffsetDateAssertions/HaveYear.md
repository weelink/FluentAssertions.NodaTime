# OffsetDateAssertions.HaveYear method

Asserts that the current OffsetDate has the specified year.

```csharp
public AndConstraint<OffsetDateAssertions> HaveYear(int year, string because = "", 
    params object[] becauseArgs)
```

| parameter | description |
| --- | --- |
| year | The year that the current OffsetDate is expected to have. |
