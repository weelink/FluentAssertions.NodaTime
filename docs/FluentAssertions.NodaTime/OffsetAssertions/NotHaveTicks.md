# OffsetAssertions.NotHaveTicks method

Asserts that the current Offset does not have the specified ticks.

```csharp
public AndConstraint<OffsetAssertions> NotHaveTicks(long ticks, string because = "", 
    params object[] becauseArgs)
```

| parameter | description |
| --- | --- |
| ticks | The ticks that the current Offset is not expected to have. |
