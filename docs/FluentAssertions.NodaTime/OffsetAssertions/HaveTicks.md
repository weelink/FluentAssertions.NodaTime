# OffsetAssertions.HaveTicks method

Asserts that the current Offset has the specified ticks.

```csharp
public AndConstraint<OffsetAssertions> HaveTicks(long ticks, string because = "", 
    params object[] becauseArgs)
```

| parameter | description |
| --- | --- |
| ticks | The ticks that the current Offset is expected to have. |
