# OffsetAssertions.NotHaveSeconds method

Asserts that the current Offset does not have the specified seconds.

```csharp
public AndConstraint<OffsetAssertions> NotHaveSeconds(int seconds, string because = "", 
    params object[] becauseArgs)
```

| parameter | description |
| --- | --- |
| seconds | The seconds that the current Offset is not expected to have. |
