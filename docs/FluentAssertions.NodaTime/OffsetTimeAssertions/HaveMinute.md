# OffsetTimeAssertions.HaveMinute method

Asserts that the current OffsetTime has the specified minute.

```csharp
public AndConstraint<OffsetTimeAssertions> HaveMinute(int minute, string because = "", 
    params object[] becauseArgs)
```

| parameter | description |
| --- | --- |
| minute | The minute that the current OffsetTime is expected to have. |
