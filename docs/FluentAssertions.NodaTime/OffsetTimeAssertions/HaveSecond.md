# OffsetTimeAssertions.HaveSecond method

Asserts that the current OffsetTime has the specified second.

```csharp
public AndConstraint<OffsetTimeAssertions> HaveSecond(int second, string because = "", 
    params object[] becauseArgs)
```

| parameter | description |
| --- | --- |
| second | The second that the current OffsetTime is expected to have. |
