# ZonedDateTimeAssertions.NotHaveNanosecondsWithinSecond method

Asserts that the current ZonedDateTime does not have the specified nanoseconds within the second.

```csharp
public AndConstraint<ZonedDateTimeAssertions> NotHaveNanosecondsWithinSecond(
    int nanosecondOfSecond, string because = "", params object[] becauseArgs)
```

| parameter | description |
| --- | --- |
| nanosecondOfSecond | The nanoseconds within the second that the current ZonedDateTime is not expected to have. |
