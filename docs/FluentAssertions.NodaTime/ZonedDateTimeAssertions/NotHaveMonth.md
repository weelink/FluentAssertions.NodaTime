# ZonedDateTimeAssertions.NotHaveMonth method

Asserts that the current ZonedDateTime does not have the specified month.

```csharp
public AndConstraint<ZonedDateTimeAssertions> NotHaveMonth(int month, string because = "", 
    params object[] becauseArgs)
```

| parameter | description |
| --- | --- |
| month | The month that the current ZonedDateTime is not expected to have. |
