# ZonedDateTimeAssertions.NotHaveTimeOfDay method

Asserts that the current ZonedDateTime does not have the specified LocalTime.

```csharp
public AndConstraint<ZonedDateTimeAssertions> NotHaveTimeOfDay(LocalTime timeOfDay, 
    string because = "", params object[] becauseArgs)
```

| parameter | description |
| --- | --- |
| timeOfDay | The LocalTime that the current ZonedDateTime is not expected to have. |
