# ZonedDateTimeAssertions.NotHaveNanosecondsWithinDay method

Asserts that the current ZonedDateTime does not have the specified nanoseconds within the day.

```csharp
public AndConstraint<ZonedDateTimeAssertions> NotHaveNanosecondsWithinDay(long nanosecondOfDay, 
    string because = "", params object[] becauseArgs)
```

| parameter | description |
| --- | --- |
| nanosecondOfDay | The nanoseconds within the day that the current ZonedDateTime is not expected to have. |
