# ZonedDateTimeAssertions.NotHaveMinute method

Asserts that the current ZonedDateTime does not have the specified minute.

```csharp
public AndConstraint<ZonedDateTimeAssertions> NotHaveMinute(int minute, string because = "", 
    params object[] becauseArgs)
```

| parameter | description |
| --- | --- |
| minute | The minute that the current ZonedDateTime is not expected to have. |
