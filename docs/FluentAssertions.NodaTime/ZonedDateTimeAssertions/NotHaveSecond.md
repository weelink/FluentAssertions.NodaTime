# ZonedDateTimeAssertions.NotHaveSecond method

Asserts that the current ZonedDateTime does not have the specified second.

```csharp
public AndConstraint<ZonedDateTimeAssertions> NotHaveSecond(int second, string because = "", 
    params object[] becauseArgs)
```

| parameter | description |
| --- | --- |
| second | The second that the current ZonedDateTime is not expected to have. |
