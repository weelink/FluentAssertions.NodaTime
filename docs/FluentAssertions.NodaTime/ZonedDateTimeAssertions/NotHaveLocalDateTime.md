# ZonedDateTimeAssertions.NotHaveLocalDateTime method

Asserts that the current ZonedDateTime does not have the specified LocalDateTime.

```csharp
public AndConstraint<ZonedDateTimeAssertions> NotHaveLocalDateTime(LocalDateTime date, 
    string because = "", params object[] becauseArgs)
```

| parameter | description |
| --- | --- |
| date | The LocalDateTime that the current ZonedDateTime is not expected to have. |
