# ZonedDateTimeAssertions.NotHaveEra method

Asserts that the current ZonedDateTime does not have the specified era.

```csharp
public AndConstraint<ZonedDateTimeAssertions> NotHaveEra(Era era, string because = "", 
    params object[] becauseArgs)
```

| parameter | description |
| --- | --- |
| era | The era that the current ZonedDateTime is not expected to have. |
