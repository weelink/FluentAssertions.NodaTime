# ZonedDateTimeAssertions.HaveMinute method

Asserts that the current ZonedDateTime has the specified minute.

```csharp
public AndConstraint<ZonedDateTimeAssertions> HaveMinute(int minute, string because = "", 
    params object[] becauseArgs)
```

| parameter | description |
| --- | --- |
| minute | The minute that the current ZonedDateTime is expected to have. |
