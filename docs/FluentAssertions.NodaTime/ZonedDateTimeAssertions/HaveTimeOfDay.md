# ZonedDateTimeAssertions.HaveTimeOfDay method

Asserts that the current ZonedDateTime has the specified LocalTime.

```csharp
public AndWhichConstraint<ZonedDateTimeAssertions, LocalTime> HaveTimeOfDay(LocalTime timeOfDay, 
    string because = "", params object[] becauseArgs)
```

| parameter | description |
| --- | --- |
| timeOfDay | The LocalTime that the current ZonedDateTime is expected to have. |
...
An AndWhichConstraint&lt;ZonedDateTimeAssertions, LocalTime&gt; which can be used to assert the LocalTime.
