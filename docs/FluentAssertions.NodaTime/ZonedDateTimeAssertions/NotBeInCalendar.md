# ZonedDateTimeAssertions.NotBeInCalendar method

Asserts that the current ZonedDateTime does not have the specified CalendarSystem.

```csharp
public AndConstraint<ZonedDateTimeAssertions> NotBeInCalendar(CalendarSystem calendar, 
    string because = "", params object[] becauseArgs)
```

| parameter | description |
| --- | --- |
| calendar | The CalendarSystem that the current ZonedDateTime is not expected to have. |
