# ZonedDateTimeAssertions.NotHaveYear method

Asserts that the current ZonedDateTime does not have the specified year.

```csharp
public AndConstraint<ZonedDateTimeAssertions> NotHaveYear(int year, string because = "", 
    params object[] becauseArgs)
```

| parameter | description |
| --- | --- |
| year | The year that the current ZonedDateTime is not expected to have. |
