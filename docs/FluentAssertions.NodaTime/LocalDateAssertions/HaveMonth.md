# LocalDateAssertions.HaveMonth method

Asserts that the current LocalDate has the specified month.

```csharp
public AndConstraint<LocalDateAssertions> HaveMonth(int month, string because = "", 
    params object[] becauseArgs)
```

| parameter | description |
| --- | --- |
| month | The month that the current LocalDate is expected to have. |
