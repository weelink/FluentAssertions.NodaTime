# LocalDateAssertions.NotHaveMonth method

Asserts that the current LocalDate does not have the specified month.

```csharp
public AndConstraint<LocalDateAssertions> NotHaveMonth(int month, string because = "", 
    params object[] becauseArgs)
```

| parameter | description |
| --- | --- |
| month | The month that the current LocalDate is not expected to have. |
