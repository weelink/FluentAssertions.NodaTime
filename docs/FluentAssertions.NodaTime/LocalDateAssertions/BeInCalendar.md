# LocalDateAssertions.BeInCalendar method

Asserts that the current LocalDate has the specified CalendarSystem.

```csharp
public AndWhichConstraint<LocalDateAssertions, CalendarSystem> BeInCalendar(
    CalendarSystem calendar, string because = "", params object[] becauseArgs)
```

| parameter | description |
| --- | --- |
| calendar | The CalendarSystem that the current LocalDate is expected to have. |
...
An AndWhichConstraint&lt;LocalDateAssertions, CalendarSystem&gt; which can be used to assert the CalendarSystem.
