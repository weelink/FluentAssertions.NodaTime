# LocalDateAssertions.HaveYear method

Asserts that the current LocalDate has the specified year.

```csharp
public AndConstraint<LocalDateAssertions> HaveYear(int year, string because = "", 
    params object[] becauseArgs)
```

| parameter | description |
| --- | --- |
| year | The year that the current LocalDate is expected to have. |
