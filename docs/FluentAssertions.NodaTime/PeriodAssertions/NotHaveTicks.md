# PeriodAssertions.NotHaveTicks method

Asserts that the current Period does not have the specified ticks.

```csharp
public AndConstraint<PeriodAssertions> NotHaveTicks(long ticks, string because = "", 
    params object[] becauseArgs)
```

| parameter | description |
| --- | --- |
| ticks | The ticks that the current Period is not expected to have. |
