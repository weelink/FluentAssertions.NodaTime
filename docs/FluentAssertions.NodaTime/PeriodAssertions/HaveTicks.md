# PeriodAssertions.HaveTicks method

Asserts that the current Period has the specified ticks.

```csharp
public AndConstraint<PeriodAssertions> HaveTicks(long ticks, string because = "", 
    params object[] becauseArgs)
```

| parameter | description |
| --- | --- |
| ticks | The ticks that the current Period is expected to have. |
