# OffsetDateTimeAssertions.NotHaveMillisecond method

Asserts that the current OffsetDateTime does not have the specified millisecond.

```csharp
public AndConstraint<OffsetDateTimeAssertions> NotHaveMillisecond(int millisecond, 
    string because = "", params object[] becauseArgs)
```

| parameter | description |
| --- | --- |
| millisecond | The millisecond that the current OffsetDateTime is not expected to have. |
