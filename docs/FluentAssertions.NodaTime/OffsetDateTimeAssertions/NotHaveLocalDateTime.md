# OffsetDateTimeAssertions.NotHaveLocalDateTime method

Asserts that the current OffsetDateTime does not have the specified LocalDateTime.

```csharp
public AndConstraint<OffsetDateTimeAssertions> NotHaveLocalDateTime(LocalDateTime date, 
    string because = "", params object[] becauseArgs)
```

| parameter | description |
| --- | --- |
| date | The LocalDateTime that the current OffsetDateTime is not expected to have. |
