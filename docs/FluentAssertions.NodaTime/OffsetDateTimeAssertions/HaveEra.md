# OffsetDateTimeAssertions.HaveEra method

Asserts that the current OffsetDateTime has the specified era.

```csharp
public AndConstraint<OffsetDateTimeAssertions> HaveEra(Era era, string because = "", 
    params object[] becauseArgs)
```

| parameter | description |
| --- | --- |
| era | The era that the current OffsetDateTime is expected to have. |
