# OffsetDateTimeAssertions.NotBeInCalendar method

Asserts that the current OffsetDateTime does not have the specified CalendarSystem.

```csharp
public AndConstraint<OffsetDateTimeAssertions> NotBeInCalendar(CalendarSystem calendar, 
    string because = "", params object[] becauseArgs)
```

| parameter | description |
| --- | --- |
| calendar | The CalendarSystem that the current OffsetDateTime is not expected to have. |
