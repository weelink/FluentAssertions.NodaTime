# OffsetDateTimeAssertions.HaveMonth method

Asserts that the current OffsetDateTime has the specified month.

```csharp
public AndConstraint<OffsetDateTimeAssertions> HaveMonth(int month, string because = "", 
    params object[] becauseArgs)
```

| parameter | description |
| --- | --- |
| month | The month that the current OffsetDateTime is expected to have. |
