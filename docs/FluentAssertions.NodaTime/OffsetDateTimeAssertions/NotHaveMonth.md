# OffsetDateTimeAssertions.NotHaveMonth method

Asserts that the current OffsetDateTime does not have the specified month.

```csharp
public AndConstraint<OffsetDateTimeAssertions> NotHaveMonth(int month, string because = "", 
    params object[] becauseArgs)
```

| parameter | description |
| --- | --- |
| month | The month that the current OffsetDateTime is not expected to have. |
