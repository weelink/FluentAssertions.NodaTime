# OffsetDateTimeAssertions.NotHaveEra method

Asserts that the current OffsetDateTime does not have the specified era.

```csharp
public AndConstraint<OffsetDateTimeAssertions> NotHaveEra(Era era, string because = "", 
    params object[] becauseArgs)
```

| parameter | description |
| --- | --- |
| era | The era that the current OffsetDateTime is not expected to have. |
