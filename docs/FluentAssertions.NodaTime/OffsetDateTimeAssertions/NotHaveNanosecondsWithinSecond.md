# OffsetDateTimeAssertions.NotHaveNanosecondsWithinSecond method

Asserts that the current OffsetDateTime does not have the specified nanoseconds within the second.

```csharp
public AndConstraint<OffsetDateTimeAssertions> NotHaveNanosecondsWithinSecond(
    int nanosecondOfSecond, string because = "", params object[] becauseArgs)
```

| parameter | description |
| --- | --- |
| nanosecondOfSecond | The nanoseconds within the second that the current OffsetDateTime is not expected to have. |
