# OffsetDateTimeAssertions.HaveMinute method

Asserts that the current OffsetDateTime has the specified minute.

```csharp
public AndConstraint<OffsetDateTimeAssertions> HaveMinute(int minute, string because = "", 
    params object[] becauseArgs)
```

| parameter | description |
| --- | --- |
| minute | The minute that the current OffsetDateTime is expected to have. |
