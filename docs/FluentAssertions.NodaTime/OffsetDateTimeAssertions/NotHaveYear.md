# OffsetDateTimeAssertions.NotHaveYear method

Asserts that the current OffsetDateTime does not have the specified year.

```csharp
public AndConstraint<OffsetDateTimeAssertions> NotHaveYear(int year, string because = "", 
    params object[] becauseArgs)
```

| parameter | description |
| --- | --- |
| year | The year that the current OffsetDateTime is not expected to have. |
