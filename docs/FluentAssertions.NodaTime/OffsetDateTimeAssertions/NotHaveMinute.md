# OffsetDateTimeAssertions.NotHaveMinute method

Asserts that the current OffsetDateTime does not have the specified minute.

```csharp
public AndConstraint<OffsetDateTimeAssertions> NotHaveMinute(int minute, string because = "", 
    params object[] becauseArgs)
```

| parameter | description |
| --- | --- |
| minute | The minute that the current OffsetDateTime is not expected to have. |
