# OffsetDateTimeAssertions.HaveSecond method

Asserts that the current OffsetDateTime has the specified second.

```csharp
public AndConstraint<OffsetDateTimeAssertions> HaveSecond(int second, string because = "", 
    params object[] becauseArgs)
```

| parameter | description |
| --- | --- |
| second | The second that the current OffsetDateTime is expected to have. |
