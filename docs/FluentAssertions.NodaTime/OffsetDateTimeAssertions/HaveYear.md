# OffsetDateTimeAssertions.HaveYear method

Asserts that the current OffsetDateTime has the specified year.

```csharp
public AndConstraint<OffsetDateTimeAssertions> HaveYear(int year, string because = "", 
    params object[] becauseArgs)
```

| parameter | description |
| --- | --- |
| year | The year that the current OffsetDateTime is expected to have. |
