# LocalDateTimeAssertions.NotHaveTicksWithinSecond method

Asserts that the current LocalDateTime does not have the specified tick of second.

```csharp
public AndConstraint<LocalDateTimeAssertions> NotHaveTicksWithinSecond(int tickOfSecond, 
    string because = "", params object[] becauseArgs)
```

| parameter | description |
| --- | --- |
| tickOfSecond | The tick of second that the current LocalDateTime is not expected to have. |
