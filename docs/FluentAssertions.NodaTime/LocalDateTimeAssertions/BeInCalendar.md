# LocalDateTimeAssertions.BeInCalendar method

Asserts that the current LocalDateTime has the specified CalendarSystem.

```csharp
public AndWhichConstraint<LocalDateTimeAssertions, CalendarSystem> BeInCalendar(
    CalendarSystem calendar, string because = "", params object[] becauseArgs)
```

| parameter | description |
| --- | --- |
| calendar | The CalendarSystem that the current LocalDateTime is expected to have. |
...
An AndWhichConstraint&lt;LocalDateTimeAssertions, CalendarSystem&gt; which can be used to assert the CalendarSystem.
