# LocalDateTimeAssertions.HaveMinute method

Asserts that the current LocalDateTime has the specified minute.

```csharp
public AndConstraint<LocalDateTimeAssertions> HaveMinute(int minute, string because = "", 
    params object[] becauseArgs)
```

| parameter | description |
| --- | --- |
| minute | The minute that the current LocalDateTime is expected to have. |
