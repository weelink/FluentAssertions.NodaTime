# LocalDateTimeAssertions.NotHaveMillisecond method

Asserts that the current LocalDateTime does not have the specified millisecond.

```csharp
public AndConstraint<LocalDateTimeAssertions> NotHaveMillisecond(int millisecond, 
    string because = "", params object[] becauseArgs)
```

| parameter | description |
| --- | --- |
| millisecond | The millisecond that the current LocalDateTime is not expected to have. |
