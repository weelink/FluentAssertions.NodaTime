# LocalDateTimeAssertions.NotHaveMinute method

Asserts that the current LocalDateTime does not have the specified minute.

```csharp
public AndConstraint<LocalDateTimeAssertions> NotHaveMinute(int minute, string because = "", 
    params object[] becauseArgs)
```

| parameter | description |
| --- | --- |
| minute | The minute that the current LocalDateTime is not expected to have. |
