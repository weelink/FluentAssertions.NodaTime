# LocalDateTimeAssertions.NotHaveEra method

Asserts that the current LocalDateTime does not have the specified era.

```csharp
public AndConstraint<LocalDateTimeAssertions> NotHaveEra(Era era, string because = "", 
    params object[] becauseArgs)
```

| parameter | description |
| --- | --- |
| era | The era that the current LocalDateTime is not expected to have. |
