# LocalDateTimeAssertions.HaveEra method

Asserts that the current LocalDateTime has the specified era.

```csharp
public AndConstraint<LocalDateTimeAssertions> HaveEra(Era era, string because = "", 
    params object[] becauseArgs)
```

| parameter | description |
| --- | --- |
| era | The era that the current LocalDateTime is expected to have. |
