# LocalDateTimeAssertions.NotHaveMonth method

Asserts that the current LocalDateTime does not have the specified month.

```csharp
public AndConstraint<LocalDateTimeAssertions> NotHaveMonth(int month, string because = "", 
    params object[] becauseArgs)
```

| parameter | description |
| --- | --- |
| month | The month that the current LocalDateTime is not expected to have. |
