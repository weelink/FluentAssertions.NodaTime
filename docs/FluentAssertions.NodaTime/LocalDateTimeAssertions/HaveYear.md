# LocalDateTimeAssertions.HaveYear method

Asserts that the current LocalDateTime has the specified year.

```csharp
public AndConstraint<LocalDateTimeAssertions> HaveYear(int year, string because = "", 
    params object[] becauseArgs)
```

| parameter | description |
| --- | --- |
| year | The year that the current LocalDateTime is expected to have. |
