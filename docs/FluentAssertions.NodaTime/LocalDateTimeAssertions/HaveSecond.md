# LocalDateTimeAssertions.HaveSecond method

Asserts that the current LocalDateTime has the specified second.

```csharp
public AndConstraint<LocalDateTimeAssertions> HaveSecond(int second, string because = "", 
    params object[] becauseArgs)
```

| parameter | description |
| --- | --- |
| second | The second that the current LocalDateTime is expected to have. |
