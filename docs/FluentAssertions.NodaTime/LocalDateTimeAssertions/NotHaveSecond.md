# LocalDateTimeAssertions.NotHaveSecond method

Asserts that the current LocalDateTime does not have the specified second.

```csharp
public AndConstraint<LocalDateTimeAssertions> NotHaveSecond(int second, string because = "", 
    params object[] becauseArgs)
```

| parameter | description |
| --- | --- |
| second | The second that the current LocalDateTime is not expected to have. |
