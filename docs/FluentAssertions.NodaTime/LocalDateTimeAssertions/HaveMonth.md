# LocalDateTimeAssertions.HaveMonth method

Asserts that the current LocalDateTime has the specified month.

```csharp
public AndConstraint<LocalDateTimeAssertions> HaveMonth(int month, string because = "", 
    params object[] becauseArgs)
```

| parameter | description |
| --- | --- |
| month | The month that the current LocalDateTime is expected to have. |
