# LocalTimeAssertions.NotHaveNanosecondsWithinDay method

Asserts that the current LocalTime does not have the specified nanoseconds within the day.

```csharp
public AndConstraint<LocalTimeAssertions> NotHaveNanosecondsWithinDay(long nanosecondOfDay, 
    string because = "", params object[] becauseArgs)
```

| parameter | description |
| --- | --- |
| nanosecondOfDay | The nanoseconds within the day that the current LocalTime is not expected to have. |
