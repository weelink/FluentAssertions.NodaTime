# LocalTimeAssertions.NotHaveNanosecondsWithinSecond method

Asserts that the current LocalTime does not have the specified nanoseconds within the second.

```csharp
public AndConstraint<LocalTimeAssertions> NotHaveNanosecondsWithinSecond(int nanosecondOfSecond, 
    string because = "", params object[] becauseArgs)
```

| parameter | description |
| --- | --- |
| nanosecondOfSecond | The nanoseconds within the second that the current LocalTime is not expected to have. |
