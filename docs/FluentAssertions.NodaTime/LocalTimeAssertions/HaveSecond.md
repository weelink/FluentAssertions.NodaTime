# LocalTimeAssertions.HaveSecond method

Asserts that the current LocalTime has the specified second.

```csharp
public AndConstraint<LocalTimeAssertions> HaveSecond(int second, string because = "", 
    params object[] becauseArgs)
```

| parameter | description |
| --- | --- |
| second | The second that the current LocalTime is expected to have. |
