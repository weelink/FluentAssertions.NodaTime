# LocalTimeAssertions.NotHaveSecond method

Asserts that the current LocalTime does not have the specified second.

```csharp
public AndConstraint<LocalTimeAssertions> NotHaveSecond(int second, string because = "", 
    params object[] becauseArgs)
```

| parameter | description |
| --- | --- |
| second | The second that the current LocalTime is not expected to have. |
