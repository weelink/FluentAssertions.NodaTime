# LocalTimeAssertions.HaveMinute method

Asserts that the current LocalTime has the specified minute.

```csharp
public AndConstraint<LocalTimeAssertions> HaveMinute(int minute, string because = "", 
    params object[] becauseArgs)
```

| parameter | description |
| --- | --- |
| minute | The minute that the current LocalTime is expected to have. |
