# DateIntervalAssertions.NotBeInCalendar method

Asserts that the current DateInterval does not have the specified CalendarSystem.

```csharp
public AndConstraint<DateIntervalAssertions> NotBeInCalendar(CalendarSystem calendar, 
    string because = "", params object[] becauseArgs)
```

| parameter | description |
| --- | --- |
| calendar | The CalendarSystem that the current DateInterval is not expected to have. |
