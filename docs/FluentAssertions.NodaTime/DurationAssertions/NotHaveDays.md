# DurationAssertions.NotHaveDays method

Asserts that the current Duration does not have the specified days.

```csharp
public AndConstraint<DurationAssertions> NotHaveDays(int days, string because = "", 
    params object[] becauseArgs)
```

| parameter | description |
| --- | --- |
| days | The days that the current Duration is not expected to have. |
