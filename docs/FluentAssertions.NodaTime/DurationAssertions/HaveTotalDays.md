# DurationAssertions.HaveTotalDays method (1 of 2)

Asserts that the current Duration has the specified total number of days.

```csharp
public AndConstraint<DurationAssertions> HaveTotalDays(double totalDays, string because = "", 
    params object[] becauseArgs)
```

| parameter | description |
| --- | --- |
| totalDays | The total number of days that the current Duration is expected to have. |
...
# DurationAssertions.HaveTotalDays method (2 of 2)

Asserts that the current Duration has the specified total number of days.

```csharp
public AndConstraint<DurationAssertions> HaveTotalDays(double totalDays, double precision, 
