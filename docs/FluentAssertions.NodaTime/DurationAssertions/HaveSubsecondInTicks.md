# DurationAssertions.HaveSubsecondInTicks method

Asserts that the current Duration has the specified subseconds in ticks.

```csharp
public AndConstraint<DurationAssertions> HaveSubsecondInTicks(int ticks, string because = "", 
    params object[] becauseArgs)
```

| parameter | description |
| --- | --- |
| ticks | The subseconds in ticks that the current Duration is expected to have. |
