# DurationAssertions.NotHaveHours method

Asserts that the current Duration does not have the specified hours.

```csharp
public AndConstraint<DurationAssertions> NotHaveHours(int hours, string because = "", 
    params object[] becauseArgs)
```

| parameter | description |
| --- | --- |
| hours | The hours that the current Duration is not expected to have. |
