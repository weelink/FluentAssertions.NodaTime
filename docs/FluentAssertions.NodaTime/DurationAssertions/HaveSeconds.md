# DurationAssertions.HaveSeconds method

Asserts that the current Duration has the specified seconds.

```csharp
public AndConstraint<DurationAssertions> HaveSeconds(int seconds, string because = "", 
    params object[] becauseArgs)
```

| parameter | description |
| --- | --- |
| seconds | The seconds that the current Duration is expected to have. |
